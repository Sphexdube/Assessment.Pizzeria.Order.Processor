...
{
    public sealed class JsonOrderFileParser : IOrderFileParser
    {
        ...
        public async Task<List<Domain.Entities.Order>> ParseOrderFileAsync(string filePath)
        {
            try
            {
                ...
                List<Domain.Entities.Order>? entries = JsonSerializer.Deserialize<List<Domain.Entities.Order>>(jsonContent, new JsonSerializerOptions { PropertyNameCaseInsensitive = true });
